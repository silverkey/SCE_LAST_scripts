...
my $DB = 'JASPAR_FAM_2008';
my $USR = 'mysql_dev';
my $PWD = 'dEvEl0pEr';
#my $VERTEBRATE = 1;
my $THRESHOLD = '80%';
my $FASTA = 'ciona_ocne.fa';

my $db = TFBS::DB::JASPAR4->connect("dbi:mysql:$DB",$USR,$PWD);
...
my $matrixset;
$matrixset = $db->get_MatrixSet();
$file .= '_jaspar_fam.txt';

#if($VERTEBRATE == 1) {
#  $matrixset = $db->get_MatrixSet(-tax_group => ['vertebrates'], -collection => ['FAM']);
#  $file .= '_jaspar_fam_vert.txt'
#}
#else {
